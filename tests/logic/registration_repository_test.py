...
class TestAddDataTarget(DBTestCase):

    def test_happy_case(self):
        actual = repo.add_data_target(
    ...
    def test_add_invalid_empty_target_type(self):
        with pytest.raises(ValueError):
            repo.add_data_target(target_type='', destination='foo')

    def test_add_invalid_empty_destination(self):
        with pytest.raises(ValueError):
            repo.add_data_target(target_type='foo', destination='')


class TestAddConsumerGroup(DBTestCase):
    ...
    def test_happy_case(self, dw_data_target):
        actual = repo.add_consumer_group('dw', dw_data_target.id)
    ...
    def test_add_consumer_group_with_non_existing_data_target(self):
        with pytest.raises(sch_exc.EntityNotFoundException):
            repo.add_consumer_group('dw', data_target_id=0)
