...
def canonicalize_sql_query(query):
    ...
    """
    return csq_limit_sub(csq_bk_sub(csq_in_sub(query))).replace('\n', ' ').replace('    ', ' ')
...
def normalize_sql_query(query):
    """Transform SQL query into a single line, normalizing whitespace for logging purposes"""
    return white_space_re.sub(' ', query)
