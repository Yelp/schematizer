...
DEFAULT_LOG_FORMAT = ('[%(process)d] %(asctime)s :: '
...
def load_package_config(filename, field='module_config'):
...
def load_default_config(config_path, env_config_path=None):
    ...
    """
    load_package_config(config_path, field='module_config')
    if env_config_path and os.path.exists(env_config_path):
        load_package_config(env_config_path, field='module_env_config')


def initialize_logging(name):
    """Configure some sensible logging basics for a service.

    If the service is running with a tty stderr, errors are logged to that.

    Logging levels can be customized by using staticconf. This is an easy way to
    increase the verbosity of your logs during development. This function
    supports the following staticconf configuration (in the default namespace):

    logging.applog.level (default :data:`logging.INFO`)
        Logging level of ``name`` logger
    logging.root.level (default :data:`logging.ERROR`)
        Logging level of the root logger
    logging.handler.console.level (default :data:`logging.WARN`)
        Logging level of the console handlers
    logging.handler.console.format
        Logging format for the console handler

    :param name: the service name. Used to configure :mod:`logging`
    :type name: string
    """
    app_log = logging.getLogger(name)
    app_log.setLevel(staticconf.read_log_level(
            'logging.applog.level',
            default='INFO'))

    if sys.stderr.isatty():
        console_handler = logging.StreamHandler()
        console_handler.setLevel(
            staticconf.read_log_level(
                    'logging.handler.console.level',
                    default='WARN'))
        format = staticconf.read_string(
                    'logging.handler.console.format',
    ...
    logging.root.setLevel(
            staticconf.read_log_level('logging.root.level', default='ERROR'))


def configure_packages(configs):
    ...
    for config in configs or []:
        ...
        if 'config' in config:
            staticconf.DictConfiguration(config['config'], namespace=config['namespace'])
...
def get_service_url(service_name, namespace='smartstack_services'):
    """Lookup a ``http://hostname:port`` string from a service name.

    :param service_name: the name of the service, as it appears in the
                         smartstack config file.
    :param namespace: the namespace used for smartstack service config.
                      Defaults to `smartstack_services`.
    :returns: a url string with http scheme, host and port
    """
    return 'http://%s:%d' % (
        get_service_host_and_port(service_name, namespace=namespace))


def get_service_host_and_port(service_name, namespace='smartstack_services'):
    """Look up ``(hostname, port)`` from a service name.
    ...
    """
    return (
        staticconf.read_string('%s.host' % service_name, namespace=namespace),
        staticconf.read_int('%s.port' % service_name, namespace=namespace)
    )
